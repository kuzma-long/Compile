        ...
        while line_length > 0:
            ...
            while max_length > 0:
                if line_length<max_length:
                    temp=line[0:line_length]
                    if temp in dict and max_length==1:  # 如果当前截取出的词能够匹配
                        result.append(temp)
                        line_length = 0
                        break
                    ...
                    temp = line[line_length - max_length:line_length]  # 截取出当前所能截取的最长的词
                    if temp == '\n':  # 单独处理行尾的换行符
                        result.append('\n')
                        line = line[0:line_length - max_length]
                        line_length -= 1
                        break
                    if temp in dict:  # 如果当前截取出的词能够匹配
                        result.append(temp)
                        line = line[0:line_length - max_length]  # 用切片将匹配出来的词切除
...
out_path = "BMM_pku_out.txt"  # 输出文件路径
